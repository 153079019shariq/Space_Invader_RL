...
class NoopResetEnv(gym.Wrapper):
    #Override the reset function so that when we reset the enviroment we do not get the same 1st state.
    #Instead we do no operation for a random number of steps until 30.
...
class EpisodicLifeEnv(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        #Although the episode will end when 1 life is gone but the ENVIROMENT IS NOT RESET.
...
class Monitor(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if done:
            ...
            #print("TOTAL_REWARD",sum(self.total_reward))
            #print("EPISODE_LENGTH",len(self.total_reward))
 

            """    
            if(sum(self.total_reward)>=900):
             print("NSTATE", observation.__array__().shape)
             print("REW",sum(self.total_reward))
             print("ACTION",action)
            """
