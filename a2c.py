...
MODEL_PATH = 'model_EpisodicLifeEnv'
        ...
        if update % log_interval == 0 or update == 1:
            ...
            if len(tr) == 100:
                ...
                if(max_rew < np.mean(tr)):
                   savepath = os.path.join(MODEL_PATH,"Space_inv_A2C_LSTM_nstep8_MAX_EpisodicLifeEnv_{}".format(start_time))
                   agent.save(savepath)
                   max_rew  = np.mean(tr)
                   print("Saved_the_max_model") 
            save_name = os.path.join(MODEL_PATH,"Space_inv_A2C_LSTM_nstep8_EpisodicLifeEnv_{}".format(start_time))
